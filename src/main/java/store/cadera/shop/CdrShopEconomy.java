...
public final class CdrShopEconomy extends JavaPlugin implements Listener {
    ...
    @Override public void onEnable() {
        try {
            ...
            loadSettings(); wallet = new Wallet();
            journal = new Journal(getDataFolder().toPath().resolve("transactions.log")); trades = new TradeEngine(journal);
            ...
            getLogger().info("CADERA / MENKIESTES | access=" + route() + " | shops=" + catalog.all().size());
    ...
    private void loadSettings() throws Exception {
        ...
        if (Files.exists(path)) {
            ...
            if (section != null) for (String key : section.getKeys(false)) {
                int npc = Integer.parseInt(key); if (npc < 0) throw new IllegalArgumentException("NPC ID negatif");
                nextBindings.put(npc, Filesafe.id(section.getString(key, "")));
            }
        }
        catalog.load();
        mode = nextMode; missing = nextMissing; distance = nextDistance; sessionSeconds = nextSeconds;
        bindings.clear(); bindings.putAll(nextBindings);
    }
    private void saveBindings(Map<Integer, String> next) throws Exception {
        YamlConfiguration yaml = new YamlConfiguration(); next.forEach((key, value) -> yaml.set("bindings." + key, value));
        Filesafe.save(yaml, getDataFolder().toPath().resolve("npcs.yml"));
        bindings.clear(); bindings.putAll(next); closeMenus();
    }
    ...
    private void npcClick(Player p, int id) {
        ...
        try { String shop = bindings.get(id); access(p, shop, id); openBuy(p, shop, id, 0); }
    ...
    private static void message(CommandSender p, String text) { p.sendMessage(color("&6[CdrShop] &f" + (text == null ? "Operasi gagal; hubungi admin." : text))); }
    private static ItemStack icon(Material type, String title, String... lore) {
        ...
        meta.setDisplayName(color(title)); meta.setLore(Arrays.stream(lore).map(CdrShopEconomy::color).toList()); item.setItemMeta(meta); return item;
    }
    private static ItemStack label(ItemStack original, String... lines) {
        ItemStack copy = original.clone(); ItemMeta meta = copy.getItemMeta();
        List<String> lore = new ArrayList<>(meta.hasLore() ? meta.getLore() : List.of());
        for (String line : lines) lore.add(color(line)); meta.setLore(lore); copy.setItemMeta(meta); return copy;
    }
    ...
    private void openShops(Player p, int page, boolean editor) {
        ...
        Menu m = menu(p, editor ? Menu.Kind.EDIT_SHOPS : Menu.Kind.SHOPS, null, null, page, editor ? "&6Editor: pilih toko" : "&6CdrShopEconomy - Toko");
        ...
        for (int i = page * 45; i < Math.min(list.size(), (page + 1) * 45); i++) {
            var shop = list.get(i); int slot = i % 45; m.keys.put(slot, shop.id());
    ...
    private void openBuy(Player p, String shopId, Integer npc, int page) {
        access(p, shopId, npc); var shop = catalog.get(shopId);
        ...
        for (int i = page * 45; i < Math.min(products.size(), (page + 1) * 45); i++) {
            var product = products.get(i); int slot = i % 45; m.keys.put(slot, product.id());
            m.inventory.setItem(slot, label(product.template(), "&aBeli/unit: $" + Money.format(product.buy()), "&7Jual/unit: " + Money.format(product.sell()), "&eKlik pilih jumlah"));
        }
        navigation(m, products.size()); m.inventory.setItem(49, icon(Material.HOPPER, "&aJual ke toko ini")); p.openInventory(m.inventory);
    }
    private Catalog.Product product(String shop, String id) {
        return catalog.get(shop).products().stream().filter(e -> e.id().equals(id)).findFirst().orElseThrow(() -> new IllegalArgumentException("Item sudah berubah; buka ulang toko."));
    }
    private void quantity(Player p, Menu source, Catalog.Product product) {
        Menu m = menu(p, Menu.Kind.QUANTITY, source.shop, source.npc, 0, "&6Pilih jumlah pembelian"); m.product = product;
        int[] choices = {1, 16, 32, 64};
        for (int i = 0; i < choices.length; i++) {
            int qty = choices[i]; m.keys.put(19 + i * 2, Integer.toString(qty));
            m.inventory.setItem(19 + i * 2, label(product.template(), "&eBeli " + qty + " item", "&aTotal: $" + Money.format(Money.total(product.buy(), qty))));
        }
        m.inventory.setItem(50, icon(Material.BARRIER, "&cBatal")); p.openInventory(m.inventory);
    }
    private void buyConfirmation(Player p, Menu source, int amount) {
        ...
        m.product = source.product; m.quantity = amount;
        m.inventory.setItem(22, label(m.product.template(), "&eJumlah: " + amount, "&aTotal: $" + Money.format(Money.total(m.product.buy(), amount))));
        m.inventory.setItem(49, icon(Material.LIME_CONCRETE, "&aKONFIRMASI BELI")); m.inventory.setItem(50, icon(Material.BARRIER, "&cBatal")); p.openInventory(m.inventory);
    }
    private void openSell(Player p, String shop, Integer npc) {
        access(p, shop, npc);
        Menu m = menu(p, Menu.Kind.SELL, shop, npc, 0, "&6Jual - pilih item inventory"); renderSell(p, m); p.openInventory(m.inventory);
    }
    private void renderSell(Player p, Menu m) {
        m.inventory.clear(); long total = 0; int count = 0;
        for (var chosen : m.selected.entrySet()) {
            Catalog.Product entry = catalog.sellMatch(p, m.shop, chosen.getValue());
            if (entry == null) continue;
            total = Math.addExact(total, Money.total(entry.sell(), chosen.getValue().getAmount())); count += chosen.getValue().getAmount();
    ...
    private void openEditor(Player p, String shop, int page) {
        ...
        for (int i = page * 45; i < Math.min(entries.size(), (page + 1) * 45); i++) {
            var e = entries.get(i); m.keys.put(i % 45, e.id());
            m.inventory.setItem(i % 45, label(e.template(), "&eID: " + e.id(), "&aBeli: " + Money.format(e.buy()), "&7Jual: " + Money.format(e.sell()), "&eKlik edit harga"));
        }
        navigation(m, entries.size()); m.inventory.setItem(49, icon(Material.PAPER, "&eKlik item inventory untuk menambah", "&7Template disalin, item asli tidak diambil.")); p.openInventory(m.inventory);
    }
    private void editPrice(Player p, String shop, Catalog.Product product) {
        ...
        m.product = product; m.buy = product.buy(); m.sell = product.sell(); renderEditor(m); p.openInventory(m.inventory);
    }
    private void renderEditor(Menu m) {
        m.inventory.setItem(4, label(m.product.template(), "&eID: " + m.product.id(), "&aBeli: " + Money.format(m.buy), "&7Jual: " + Money.format(m.sell)));
    ...
    public void click(InventoryClickEvent e) {
        ...
        Bukkit.getScheduler().runTask(this, () -> {
            ...
            try {
                if (System.currentTimeMillis() > m.expires) { p.closeInventory(); throw new IllegalArgumentException("Sesi habis. Silakan buka ulang."); }
                if (m.editor()) {
                ...
                switch (m.kind) {
                    case SHOPS, EDIT_SHOPS -> {
                        if (key != null) { if (m.editor()) openEditor(p, key, 0); else openBuy(p, key, null, 0); }
                        else if (raw == 49 && !m.editor()) openSell(p, null, null);
                        else if ((raw == 45 || raw == 53) && m.inventory.getItem(raw) != null) openShops(p, m.page + (raw == 45 ? -1 : 1), m.editor());
                    }
                    case BUY -> {
                        if (key != null) quantity(p, m, product(m.shop, key));
                        else if (raw == 49) openSell(p, m.shop, m.npc);
                        else if ((raw == 45 || raw == 53) && m.inventory.getItem(raw) != null) openBuy(p, m.shop, m.npc, m.page + (raw == 45 ? -1 : 1));
                    }
                    case QUANTITY -> { if (key != null) buyConfirmation(p, m, Integer.parseInt(key)); }
                    case BUY_CONFIRM -> { if (raw == 49) { p.closeInventory(); buy(p, m); } }
                    case SELL -> {
                        if (raw < 36) { m.selected.remove(raw); renderSell(p, m); }
                        else if (raw == 48) {
                            m.selected.clear();
                            for (int i = 0; i < 36; i++) { ItemStack stack = p.getInventory().getItem(i); if (catalog.sellMatch(p, m.shop, stack) != null) m.selected.put(i, stack.clone()); }
                            renderSell(p, m);
                        } else if (raw == 49) { p.closeInventory(); sell(p, m.shop, m.npc, m.selected); }
                    }
                    case EDIT_ITEMS -> {
                        if (key != null) editPrice(p, m.shop, product(m.shop, key));
                        else if ((raw == 45 || raw == 53) && m.inventory.getItem(raw) != null) openEditor(p, m.shop, m.page + (raw == 45 ? -1 : 1));
                    }
                    case EDIT_PRICE -> {
                        if (raw == 49) {
                            catalog.saveProduct(m.shop, new Catalog.Product(m.product.id(), m.product.template(), m.buy, m.sell));
                            closeMenus(); message(p, "Item tersimpan: " + m.product.id()); openEditor(p, m.shop, 0);
                        } else if (raw >= 19 && raw <= 23) { m.buy = adjust(m.buy, raw - 19); renderEditor(m); }
                        else if (raw >= 28 && raw <= 32) { m.sell = adjust(m.sell, raw - 28); renderEditor(m); }
                    }
                }
    ...
    private void bottomClick(Player p, Menu m, int slot) {
        ...
        if (m.kind == Menu.Kind.SELL) {
            if (m.selected.containsKey(slot)) m.selected.remove(slot);
                ...
                if (catalog.sellMatch(p, m.shop, stack) == null) throw new IllegalArgumentException("Item ini tidak diterima toko.");
                m.selected.put(slot, stack.clone());
            }
            renderSell(p, m);
        } else if (m.kind == Menu.Kind.EDIT_ITEMS) {
            ItemStack template = stack.clone(); template.setAmount(1);
    ...
    @EventHandler public void quit(PlayerQuitEvent e) { if (e.getPlayer().getOpenInventory().getTopInventory().getHolder() instanceof Menu) e.getPlayer().closeInventory(); }

    private static ItemStack[] copy(ItemStack[] input) { return Arrays.stream(input).map(i -> i == null ? null : i.clone()).toArray(ItemStack[]::new); }
    private void sell(Player p, String shop, Integer npc, Map<Integer, ItemStack> chosen) throws Exception {
        access(p, shop, npc); if (chosen.isEmpty()) throw new IllegalArgumentException("Belum ada item yang dipilih.");
        ItemStack[] before = copy(p.getInventory().getStorageContents()), after = copy(before); long total = 0; int quantity = 0;
        for (var entry : chosen.entrySet()) {
            int slot = entry.getKey(); ItemStack expected = entry.getValue();
            if (slot < 0 || slot >= before.length || !Objects.equals(before[slot], expected)) throw new IllegalArgumentException("Inventory berubah. Buka ulang dan pilih kembali item.");
            var product = catalog.sellMatch(p, shop, expected);
            if (product == null) throw new IllegalArgumentException("Barang/hak akses berubah; transaksi dibatalkan.");
            total = Math.addExact(total, Money.total(product.sell(), expected.getAmount())); quantity += expected.getAmount(); after[slot] = null;
        }
        execute(p, before, after, total, true, quantity, shop);
    }
    private void buy(Player p, Menu m) throws Exception {
        access(p, m.shop, m.npc); var current = product(m.shop, m.product.id());
        if (current.buy() != m.product.buy() || !current.template().isSimilar(m.product.template())) throw new IllegalArgumentException("Harga/item berubah; buka ulang toko.");
        ItemStack[] before = copy(p.getInventory().getStorageContents()), after = copy(before);
        ...
        for (int i = 0; i < after.length && remaining > 0; i++) {
            ...
            int add = Math.min(space, remaining); after[i].setAmount(after[i].getAmount() + add); remaining -= add;
        }
        for (int i = 0; i < after.length && remaining > 0; i++) if (after[i] == null || after[i].getType().isAir()) {
            after[i] = current.template().clone(); int add = Math.min(remaining, Math.min(after[i].getMaxStackSize(), p.getInventory().getMaxStackSize())); after[i].setAmount(add); remaining -= add;
        }
        if (remaining > 0) throw new IllegalArgumentException("Ruang inventory tidak cukup. Tidak ada uang dipotong.");
        execute(p, before, after, Money.total(current.buy(), m.quantity), false, m.quantity, m.shop);
    }
    private void execute(Player p, ItemStack[] before, ItemStack[] after, long amount, boolean credit, int count, String shop) throws Exception {
        ...
        try {
            ...
            if (result == TradeEngine.Result.DECLINED) message(p, "Transaksi ditolak economy (saldo kurang/batas saldo). Item dikembalikan.");
            else message(p, "Berhasil " + (credit ? "menjual " : "membeli ") + count + " item, total $" + Money.format(amount));
        } catch (Exception ex) {
    ...
    private static String encode(ItemStack[] storage) { return Base64.getEncoder().encodeToString(ItemStack.serializeItemsAsBytes(Arrays.asList(storage))); }
    ...
    @Override public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        try {
            ...
            access(p, null, null);
            switch (cmd.getName()) {
                case "toko" -> { if (args.length == 0) openShops(p, 0, false); else if (args.length == 1) openBuy(p, args[0], null, 0); else throw new IllegalArgumentException("/toko [nama-toko]"); }
                case "jualgui" -> { if (args.length != 0) throw new IllegalArgumentException("/jualgui"); openSell(p, null, null); }
                case "jual" -> {
    ...
    private void admin(CommandSender sender, String[] a) throws Exception {
        ...
        if (a.length == 0) {
            ...
            message(sender, "Console: pending | resolve <tx-UUID> reviewed"); return;
        }
        switch (a[0].toLowerCase(Locale.ROOT)) {
            case "create" -> { require(a, 2); catalog.create(a[1]); closeMenus(); message(sender, "Toko dibuat: " + a[1]); }
            case "editor" -> { Player p = player(sender); if (a.length == 1) openShops(p, 0, true); else { require(a, 2); openEditor(p, a[1], 0); } }
            case "add" -> {
                require(a, 5); Player p = player(sender); ItemStack item = p.getInventory().getItemInMainHand().clone();
                if (item.getType().isAir()) throw new IllegalArgumentException("Pegang item template.");
                if (catalog.get(a[1]).products().stream().anyMatch(e -> e.id().equals(a[2]))) throw new IllegalArgumentException("ID sudah ada; gunakan editor/price.");
                item.setAmount(1); catalog.saveProduct(a[1], new Catalog.Product(a[2], item, Money.parse(a[3]), Money.parse(a[4])));
                closeMenus(); message(sender, "Template disalin dan disimpan.");
            }
            case "price" -> {
                require(a, 5); var old = product(a[1], a[2]);
                catalog.saveProduct(a[1], new Catalog.Product(old.id(), old.template(), Money.parse(a[3]), Money.parse(a[4])));
                closeMenus(); message(sender, "Harga diperbarui.");
            }
            case "remove" -> { require(a, 3); catalog.remove(a[1], a[2]); closeMenus(); message(sender, "Listing dihapus; item pemain tidak disentuh."); }
            case "bind" -> {
                require(a, 3); catalog.get(a[1]); int id = Integer.parseInt(a[2]);
                ...
                Map<Integer, String> next = new LinkedHashMap<>(bindings); next.put(id, a[1]); saveBindings(next); message(sender, "NPC " + id + " terhubung ke " + a[1] + "; akses=" + route());
            }
            case "unbind" -> { require(a, 2); Map<Integer, String> next = new LinkedHashMap<>(bindings); if (next.remove(Integer.parseInt(a[1])) == null) throw new IllegalArgumentException("Binding tidak ditemukan."); saveBindings(next); message(sender, "Binding dilepas; akses=" + route()); }
            case "reload" -> { require(a, 1); closeMenus(); loadSettings(); message(sender, "Konfigurasi valid dimuat. Akses=" + route()); }
            case "status" -> message(sender, "v1.0.0-beta.1 | akses=" + route() + " | toko=" + catalog.all().size() + " | NPC=" + bindings + " | economy=" + wallet.available());
            case "pending" -> { console(sender); message(sender, "Transaksi untuk review: " + journal.pending()); }
            case "resolve" -> {
                console(sender); require(a, 3); if (!a[2].equals("reviewed")) throw new IllegalArgumentException("Review uang/item di journal dahulu; argumen akhir: reviewed");
                journal.finish(UUID.fromString(a[1]), "RESOLVED", "console-reviewed"); message(sender, "Kunci dilepas. Tidak ada refund atau pemberian item otomatis.");
            }
            default -> throw new IllegalArgumentException("Subcommand tidak dikenal. Gunakan /cdrshop.");
        }
    }
    private static void console(CommandSender sender) { if (!(sender instanceof ConsoleCommandSender)) throw new IllegalArgumentException("Hanya console server."); }
    private static Player player(CommandSender sender) { if (sender instanceof Player p) return p; throw new IllegalArgumentException("Gunakan di dalam game."); }
    private static void require(String[] a, int size) { if (a.length != size) throw new IllegalArgumentException("Argumen salah; lihat /cdrshop."); }
    @Override public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        if (cmd.getName().equals("cdrshop") && sender.hasPermission("cdrshopeconomy.admin")) {
            if (args.length == 1) options.addAll(List.of("create", "editor", "add", "price", "remove", "bind", "unbind", "reload", "status", "pending", "resolve"));
            if (args.length == 2 && Set.of("editor", "add", "price", "remove", "bind").contains(args[0])) catalog.all().forEach(s -> options.add(s.id()));
        }
